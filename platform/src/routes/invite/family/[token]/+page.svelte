<script lang="ts">
	...
	import { acceptFamilyInvite, getFamilyInviteByToken } from '$lib/api/family.remote';
	...
	const invite = $derived(await getFamilyInviteByToken({ token: params.token }));
	...
	let submitting = $derived(!!acceptFamilyInvite.pending);
</script>

<svelte:head>
	<title>Family Invitation | {PUBLIC_APP_NAME}</title>
</svelte:head>
	...
	<div class="w-full max-w-md space-y-6 rounded-2xl border p-6 shadow-sm">
			...
			<div>
				...
				<p class="mt-2 text-sm text-muted-foreground">
					{#if invite.reason === 'expired'}
						This invitation to follow {invite.playerName} has expired. Ask your league organizer to
						resend it.
		...
		{:else}
			<div>
				...
				<p class="mt-2 text-sm text-muted-foreground">
					Follow <strong>{invite.playerName}</strong> on
					<strong>{invite.teamName}</strong> in <strong>{invite.leagueName}</strong>.
					Create or sign in to see their GameChanger player page.
				</p>
				{#if invite.email}
			...
			{:else if user && invite.email && user.email.toLowerCase() !== invite.email.toLowerCase()}
			...
			{:else}
				<form
					{...acceptFamilyInvite.enhance(async ({ submit }) => {
						await submit();
					})}
				>
					<input {...acceptFamilyInvite.fields.token.as('hidden', params.token)} />
					<ErrorAlert errors={acceptFamilyInvite.fields.issues()} />
