...
class RuleBasedAssistant:
    ...
    def _team_scout_report(self, team: str) -> str:
        ...
        if team_stats["PTS"] > league["PTS"] * 1.05:
            strengths.append(
                f"Scoring (+{pct_diff(team_stats['PTS'], league['PTS']) * 100:.0f}% vs league)"
            )
        ...
        if team_stats["Three_pct"] > league["Three_pct"] * 1.05:
            strengths.append(
                f"Three-point shooting (3P% {team_stats['Three_pct']:.1%})"
            )

        # Playmaking
        if team_stats["AST"] > league["AST"] * 1.05:
            strengths.append(
                f"Ball movement (AST +{pct_diff(team_stats['AST'], league['AST']) * 100:.0f}% vs league)"
            )
        if team_stats["TOV"] > league["TOV"] * 1.10:
            weaknesses.append(
                f"Turnovers (TOV +{pct_diff(team_stats['TOV'], league['TOV']) * 100:.0f}% vs league)"
            )

        # Rebounding / efficiency
        if team_stats["REB"] > league["REB"] * 1.05:
            strengths.append(
                f"Rebounding (REB +{pct_diff(team_stats['REB'], league['REB']) * 100:.0f}% vs league)"
            )
        if team_stats["Efficiency"] > league["Efficiency"] * 1.05:
            strengths.append(
                f"Overall efficiency (+{pct_diff(team_stats['Efficiency'], league['Efficiency']) * 100:.0f}% vs league)"
            )
        if team_stats["Efficiency"] < league["Efficiency"] * 0.95:
            weaknesses.append(
                f"Overall efficiency (-{abs(pct_diff(team_stats['Efficiency'], league['Efficiency']) * 100):.0f}% vs league)"
            )
        ...
        lines.append(
            f"- PTS: {team_stats['PTS']:.1f} | REB: {team_stats['REB']:.1f} | AST: {team_stats['AST']:.1f} | TOV: {team_stats['TOV']:.1f} | EFF: {team_stats['Efficiency']:.1f}"
        )
        lines.append(
            f"- FG%: {team_stats['FG_pct']:.1%} | 3P%: {team_stats['Three_pct']:.1%}"
        )
        lines.append("")
    ...
    def _get_team_leaders(
        self, team: str, stat_category: str, top_n: int = 5
    ) -> List[Dict[str, Any]]:
        ...
        team_players = [
            p
            for p in data_manager.get_all_players(season=self._data_season)
            if p.get("team") == team
        ]
        rows: List[Dict[str, Any]] = []

        for p in team_players:
            prof = data_manager.get_player_profile(
                p["player_no"], p["team"], season=self._data_season
            )
            if "error" in prof:
                continue
            ...
            if stat_category == "points":
                ...
                pg = game_data[
                    (game_data["Player No."] == p["player_no"])
                    & (game_data["Team"] == team)
                ]
    ...
    def _top5_all_categories(self, team: Optional[str] = None) -> str:
        ...
        if team:
            ...
            for title, cat in categories:
                parts.append(
                    self._format_leaderboard(title, self._get_league_leaders(cat, 5))
                )
                parts.append("")
    ...
    def get_personalized_feedback(self, player_profile: Dict[str, Any]) -> str:
        ...
        feedback_parts.append(
            f"## Coaching Analysis for Player {player_profile.get('player_no')} ({player_profile.get('team')})"
        )
        feedback_parts.append("")
        feedback_parts.append(
            f"Based on {player_profile.get('total_games')} games played, here's your personalized coaching analysis:"
        )
        feedback_parts.append("")
        ...
        if strengths:
            ...
            for strength in strengths:
                ...
                if "Scoring" in strength or "scoring" in strength:
                    ...
                    if avg_pts >= 15:
                        feedback_parts.append(
                            f"  - You're averaging {avg_pts:.1f} points per game - excellent offensive production!"
                        )
                        feedback_parts.append(
                            f"  - Use your scoring ability to draw defenders and create opportunities for teammates"
                        )
                elif "Rebounding" in strength or "rebounding" in strength:
                    avg_reb = stats.get("rebounds", {}).get("average", 0)
                    feedback_parts.append(
                        f"  - Your {avg_reb:.1f} rebounds per game show strong court positioning"
                    )
                    feedback_parts.append(
                        f"  - Continue boxing out and tracking the ball off the rim"
                    )
                elif "Passing" in strength or "playmaking" in strength:
                    avg_ast = stats.get("assists", {}).get("average", 0)
                    feedback_parts.append(
                        f"  - With {avg_ast:.1f} assists per game, you're an effective playmaker"
                    )
                    feedback_parts.append(
                        f"  - Keep looking for cutters and open shooters"
                    )
            feedback_parts.append("")
        ...
        if weaknesses:
            ...
            for weakness in weaknesses:
                ...
                if "Turnover" in weakness or "Ball control" in weakness:
                    ...
                    feedback_parts.append(
                        f"  - Currently averaging {avg_tov:.1f} turnovers per game"
                    )

                    # Get specific drills from library
                    dribbling_drills = drill_library.get_drills_for_category(
                        "dribbling"
                    )
                    for i, drill in enumerate(
                        dribbling_drills[:3], 1
                    ):  # Top 3 dribbling drills
                        feedback_parts.append(
                            f"  - **Drill {i}**: {drill['name']} - {drill['description']}"
                        )
                        feedback_parts.append(f"    Duration: {drill['duration']}")

                    feedback_parts.append(
                        f"  - **Game tip**: Slow down when making decisions, especially in traffic"
                    )
                    ...
                    feedback_parts.append(
                        f"  - Current shooting: FG {fg_pct:.1%}, 3PT {three_pct:.1%}"
                    )
                    ...
                    avg_reb = stats.get("rebounds", {}).get("average", 0)
                    feedback_parts.append(
                        f"  - Current average: {avg_reb:.1f} rebounds per game"
                    )

                    # Get specific rebounding drills from library
                    rebounding_drills = drill_library.get_drills_for_category(
                        "rebounding"
                    )
                    for i, drill in enumerate(
                        rebounding_drills[:2], 1
                    ):  # Top 2 rebounding drills
                        feedback_parts.append(
                            f"  - **Drill {i}**: {drill['name']} - {drill['description']}"
                        )
                        feedback_parts.append(f"    Duration: {drill['duration']}")
                    ...
                    feedback_parts.append(
                        f"  - **Game tip**: Position yourself between opponent and basket before shot goes up"
                    )

                elif "Playmaking" in weakness or "assists" in weakness:
                    avg_ast = stats.get("assists", {}).get("average", 0)
                    feedback_parts.append(
                        f"  - Current average: {avg_ast:.1f} assists per game"
                    )
                    ...
                    feedback_parts.append(
                        f"  - **Game tip**: Keep your head up and scan the court for open teammates"
                    )
        ...
        if fg_pct < 0.40:
            feedback_parts.append(
                "1. **Focus on shot selection**: Work on taking higher-percentage shots. Practice finishing at the rim and taking open shots within your range."
            )
        elif fg_pct >= 0.45:
            feedback_parts.append(
                "1. **Maintain efficient shooting**: Your field goal percentage is strong. Continue taking quality shots and keep practicing."
            )
        else:
            feedback_parts.append(
                "1. **Balance shot quality and quantity**: Look for opportunities to improve your field goal percentage through better shot selection."
            )
        ...
        if efficiency < 10:
            feedback_parts.append(
                "2. **Improve overall efficiency**: Focus on reducing turnovers and improving shot selection. Every possession matters."
            )
        elif efficiency >= 15:
            feedback_parts.append(
                "2. **Maintain high efficiency**: You're playing efficient basketball. Keep making smart decisions on both ends of the court."
            )
        else:
            feedback_parts.append(
                "2. **Continue building efficiency**: You're on the right track. Look for small improvements in decision-making."
            )

        # Balance recommendations
        if strengths and weaknesses:
            feedback_parts.append(
                "3. **Build on strengths while addressing weaknesses**: Use your strengths to create advantages while actively working on your areas for improvement during practice."
            )
        ...
        if advanced_stats:
            ...
            feedback_parts.append(
                f"- **Assist/Turnover Ratio**: {ast_tov:.2f} - {'Excellent playmaking!' if ast_tov >= 2.0 else 'Room for improvement in ball control'}"
            )
            feedback_parts.append(
                f"- **True Shooting %**: {ts_pct:.1f}% - {'Efficient scorer!' if ts_pct >= 50 else 'Focus on shot quality'}"
            )
            feedback_parts.append(
                f"- **Rebound %**: {reb_pct:.1f}% - {'Strong rebounder!' if reb_pct >= 10 else 'Work on positioning and timing'}"
            )
    ...
    def _get_league_leaders(
        self, stat_category: str, top_n: int = 5
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for player in players:
                profile = data_manager.get_player_profile(
                    player["player_no"], player["team"], season=self._data_season
                )
                if "error" in profile:
                    continue
                ...
                if stat_category == "fg_pct" or stat_category == "field goal":
                    player_info["stat_value"] = stats.get("shooting", {}).get(
                        "fg_pct", 0
                    )
                    player_info["stat_display"] = f"{player_info['stat_value']:.1%}"
                    player_info["stat_name"] = "Field Goal %"
                elif (
                    stat_category == "three_point"
                    or stat_category == "3pt"
                    or stat_category == "3ptm"
                ):
                    ...
                    total_3ptm = (
                        int(player_games["3PTM"].sum()) if not player_games.empty else 0
                    )
                    ...
                    player_info["stat_value"] = stats.get("points", {}).get(
                        "average", 0
                    )
    ...
    def get_ai_response(
        self, user_question: str, player_profile: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        ...
        if player_profile is None:
            ...
            if team and any(
                k in question_lower
                for k in ["strength", "weakness", "scout", "report", "breakdown"]
            ):
            ...
            if team and any(
                k in question_lower
                for k in ["best player", "best on", "top player", "mvp"]
            ):
                best = self._best_player_on_team(team)
                if not best:
                    return (
                        f"I couldn't determine the best player on {team} from the data."
                    )
                lines = []
            ...
            is_league_question = (
                any(word in question_lower for word in ["who", "which player"])
                and any(
                    word in question_lower for word in ["best", "most", "top", "leader"]
                )
            ) or (
                any(word in question_lower for word in ["best", "most", "top"])
                and any(
            ...
            if is_league_question:
                ...
                if team:
                    if any(
                        ...
                        return self._format_leaderboard(
                            f"Top Field Goal Shooters — {team}",
                            self._get_team_leaders(team, "fg_pct", 5),
                        )
                    if any(
                        word in question_lower
                        for word in [
                        ...
                        if (
                            ...
                            or "pct" in question_lower
                        ):
                            return self._format_leaderboard(
                                f"Top Three-Point % — {team}",
                                self._get_team_leaders(team, "three_pct", 5),
                            )
                        return self._format_leaderboard(
                            f"Most 3PT Made — {team}",
                            self._get_team_leaders(team, "three_point", 5),
                        )
                    if any(
                        word in question_lower
                        for word in ["points", "scoring", "scorer", "ppg"]
                    ):
                        return self._format_leaderboard(
                            f"Top Scorers — {team}",
                            self._get_team_leaders(team, "points", 5),
                        )
                    if any(
                        word in question_lower
                        for word in ["rebound", "rebounds", "rpg"]
                    ):
                        return self._format_leaderboard(
                            f"Top Rebounders — {team}",
                            self._get_team_leaders(team, "rebounds", 5),
                        )
                    if any(
                        word in question_lower
                        for word in ["assists", "passing", "playmaking", "apg"]
                    ):
                        return self._format_leaderboard(
                            f"Top Playmakers — {team}",
                            self._get_team_leaders(team, "assists", 5),
                        )
                    if any(
                        word in question_lower for word in ["efficiency", "efficient"]
                    ):
                        return self._format_leaderboard(
                            f"Most Efficient — {team}",
                            self._get_team_leaders(team, "efficiency", 5),
                        )
                ...
                if any(
                    ...
                    if leaders:
                        return self._format_leaderboard(
                            "Top Field Goal Shooters", leaders
                        )
                ...
                # Three-point made questions
                    ...
                    if leaders:
                        return self._format_leaderboard(
                            f"Top {stat_label} Leaders", leaders
                        )
                ...
                # Rebounding questions
                    ...
                    word in question_lower for word in ["rebound", "rebounds", "rpg"]
                ):
                ...
                # Efficiency questions
                    ...
                    if leaders:
                        return self._format_leaderboard(
                            "Most Efficient Players", leaders
                        )
        ...
        if any(
            ...
            if player_profile:
                ...
                if "form shooting" in question_lower:
                    ...
                    "catch and shoot" in question_lower
                    or "catch-and-shoot" in question_lower
                ):
                    ...
                    for i, drill in enumerate(shooting_drills[:4], 1):
                        response_parts.append(
                            f"**{i}. {drill['name']}** ({drill['difficulty']})"
                        )
                    ...
                    if fg_pct < 0.40:
                        response_parts.append(
                            "**Quick Tip:** Focus on shot selection - take open shots within your range"
                        )
            else:
                shooting_drills = drill_library.get_drills_for_category("shooting")
                ...
                for i, drill in enumerate(shooting_drills[:3], 1):
                    response_parts.append(
                        f"{i}. **{drill['name']}** - {drill['description']}"
                    )
                response_parts.append("")
                response_parts.append(
                    "Ask for a specific drill to see detailed instructions!"
                )
        ...
        # Rebounding questions
            ...
            if player_profile:
                ...
                response_parts.append(
                    f"You're averaging {avg_reb:.1f} rebounds per game."
                )
                response_parts.append("")
                if avg_reb < 5:
                    response_parts.append(
                        "**To improve rebounding, try these drills:**"
                    )
                else:
                    response_parts.append(
                        "**To maintain strong rebounding, practice these drills:**"
                    )
            else:
            ...
            for i, drill in enumerate(rebounding_drills[:3], 1):
                response_parts.append(
                    f"**{i}. {drill['name']}** ({drill['difficulty']}, {drill['duration']})"
                )
                response_parts.append(f"   {drill['description']}")
                response_parts.append("")

            response_parts.append(
                "💡 **Key Tips:** Box out positioning, watch the ball's trajectory, pursue aggressively!"
            )
            response_parts.append(
                "Ask for a specific drill name to see detailed instructions!"
            )
        ...
        # Turnover/ball control questions
            ...
            if player_profile:
                ...
                if any(
                    "passing" in s.lower() or "playmaking" in s.lower()
                    for s in strengths
                ):
                    response_parts.append(
                        "**✅ Playmaking is one of your strengths!** Focus on reducing turnovers to maximize this."
                    )
                    response_parts.append("")

                response_parts.append(
                    f"You're averaging {avg_tov:.1f} turnovers per game."
                )
                response_parts.append("")
            ...
            if "right hand" in question_lower:
                ...
                for i, drill in enumerate(dribbling_drills[:4], 1):
                    ...
                    response_parts.append("")
                response_parts.append(
                    "💡 **Tip:** Ask for a specific drill name to see detailed step-by-step instructions!"
                )
        ...
        # General improvement or questions about strengths
                    ...
                    if strengths:
                        ...
                        for strength in strengths:
                            ...
                            if "Scoring" in strength or "scoring" in strength:
                                avg_pts = stats.get("points", {}).get("average", 0)
                                response_parts.append(
                                    f"  • Averaging {avg_pts:.1f} points per game"
                                )
                                response_parts.append(
                                    f"  • Use this to draw defenders and create opportunities for teammates"
                                )
                            elif "Rebounding" in strength or "rebounding" in strength:
                                avg_reb = stats.get("rebounds", {}).get("average", 0)
                                response_parts.append(
                                    f"  • {avg_reb:.1f} rebounds per game - strong court positioning"
                                )
                            elif "Passing" in strength or "playmaking" in strength:
                                avg_ast = stats.get("assists", {}).get("average", 0)
                                response_parts.append(
                                    f"  • {avg_ast:.1f} assists per game - effective playmaking"
                                )
                            elif "Three-point" in strength or "three-point" in strength:
                                three_pct = stats.get("shooting", {}).get(
                                    "three_pct", 0
                                )
                                response_parts.append(
                                    f"  • {three_pct:.1%} three-point shooting - keep it up!"
                                )
                            elif "Shooting" in strength or "shooting" in strength:
                                fg_pct = stats.get("shooting", {}).get("fg_pct", 0)
                                response_parts.append(
                                    f"  • {fg_pct:.1%} field goal percentage - efficient scorer"
                                )
                            response_parts.append("")

                        response_parts.append(
                            "💡 **Tip:** Build on these strengths while working on areas for improvement!"
                        )
                    else:
                        response_parts.append(
                            "Keep working hard! Focus on fundamental skills like shooting, ball handling, and defense."
                        )
            ...
            if player_profile:
                    ...
                    for weakness in weaknesses[:2]:  # Top 2 weaknesses
                        ...
                        if drills:
                            ...
                            for drill in drills[:2]:  # Top 2 drills per weakness
                                response_parts.append(
                                    f"- **{drill['name']}** ({drill['difficulty']}, {drill['duration']})"
                                )
                                response_parts.append(f"  {drill['description']}")
                            response_parts.append("")

                    response_parts.append(
                        "💡 **Tip:** Ask for a specific drill name like 'Right Hand Dribbling' to see detailed instructions!"
                    )
                else:
                    response_parts.append("**Key areas to work on:**")
                    response_parts.append(
                        "1. **Practice fundamentals daily** - Shooting, dribbling, passing"
                    )
                    response_parts.append(
                        "2. **Study game film** - Watch your games and identify patterns"
                    )
                    response_parts.append(
                        "3. **Conditioning** - Basketball requires endurance and agility"
                    )
                    response_parts.append("")
                ...
                response_parts.append("")
                response_parts.append(
                    "💡 **Tip:** Ask for a specific drill name to see detailed instructions!"
                )

        # Drill-specific requests (catch all drill name queries)
        elif any(
            drill_name.lower() in question_lower
            for drill_name in [
            ...
            if found_drill:
                ...
                response_parts.append(
                    "I can help you find specific drills! Try asking:"
                )
        ...
        # Default response
            ...
            if self._data_season == 3:
                response_parts.append(
                    "I’m your **3 on 3 tournament** assistant. I use this dashboard’s tournament stats (Round Robin / Playoffs) and the same drill library."
                )
                response_parts.append("")
                response_parts.append("I can help with:")
                response_parts.append(
                    "- **3 on 3 strategy** — spacing, ball screens, quick decisions, switching defense, offensive rebounds in a short shot clock"
                )
                response_parts.append(
                    "- **GCIR / GCMVP** — what they measure on this site (see tournament section tooltips)"
                )
                response_parts.append(
                    "- **Shooting, rebounding, turnovers** — ask for drills by topic"
                )
                response_parts.append(
                    "- **Team questions** — “*Team name* strengths and weaknesses”, “best player on *Team*”"
                )
                response_parts.append(
                    "- **Leaderboards** — “top scorers”, “most threes”, “top 5 in each category”"
                )
                response_parts.append("")
                response_parts.append("**Try asking:**")
                response_parts.append(
                    "- “How should we defend ball screens in 3 on 3?”"
                )
                response_parts.append(
                    "- “What’s a good warmup for a 3 on 3 tournament?”"
                )
                ...
                response_parts.append(
                    "- **Shooting improvement** - Try asking 'shooting drills'"
                )
                response_parts.append("- **Rebounding tips** - Ask 'rebounding drills'")
                response_parts.append(
                    "- **Ball control and turnovers** - Ask 'dribbling drills'"
                )
                response_parts.append(
                    "- **Specific drills** - Ask for drill names like 'Right Hand Dribbling'"
                )
                response_parts.append(
                    "- **Coach / scout questions (All Players)** - Ask things like 'best player on Black team' or 'Black team strengths and weaknesses'"
                )
                response_parts.append(
                    "- **Leaderboards (All Players)** - Ask 'best FG shooter', 'most 3PTs made', or 'top 5 in each category'"
                )
                response_parts.append("")
                response_parts.append("**Try asking:**")
