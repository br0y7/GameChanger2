...
def _pick_assistant() -> tuple[AIAssistant, str]:
    """
...
def render_3on3_ai_chat_interface(player_profile=None):
    ...
    if assistant_type == "AI":
        ...
        st.markdown(
            "Ask about **this tournament**, strategy for 3 on 3, drills, or who’s leading in the stats."
        )
        st.info(
            "Using OpenAI. Switch to **Rule-Based** in the sidebar if you prefer no API calls."
        )
    elif assistant_type == "Hybrid":
        st.subheader("🏀 3 on 3 Hybrid Assistant")
        st.markdown(
            "Rule-based answers use **tournament data**; OpenAI refines the reply with 3 on 3 context."
        )
    else:
        st.subheader("🏀 3 on 3 Tournament Assistant")
        st.markdown(
            "Tips and stats are tuned for **3 on 3** (space, pace, GCIR/GCMVP on this dashboard)."
        )
    ...
    if prompt := st.chat_input(
        "Ask about this 3 on 3 tournament...", key="chat_input_3on3"
    ):
        ...
        with st.chat_message("assistant"):
            with st.spinner(
                ...
                else "Analyzing tournament data..."
            ):
                if assistant_type == "Hybrid":
                    draft = tournament_3on3_assistant.get_ai_response(
                    ...
                    openai_prompt = f"{ctx}\n\n---\n\nUser question:\n{prompt}"
                    response = coaching_assistant.get_ai_response(
                        openai_prompt, player_profile
                    )
                else:
                    response = tournament_3on3_assistant.get_ai_response(
                        prompt, player_profile
                    )
                st.write(response)
...
def render_ai_chat_interface(player_profile: PlayerProfile | None = None):
    """
    ...
    st.write(type(coaching_assistant).__name__)
    ...
    if assistant_type == "AI":
        ...
        st.info(
            "Using OpenAI (paid). If you don’t want to pay, switch Assistant mode to Rule-Based in the sidebar."
        )
    elif assistant_type == "Hybrid":
        st.header("🤖 Hybrid Coaching Assistant")
        st.markdown("Combines free rule-based analysis with OpenAI refinement.")
        st.info(
            "OpenAI is enabled for refinement; rule-based output is used as context."
        )
    elif assistant_type == "test":
        st.header("🤖 Ollama ")
        st.markdown("Ask about stuff.")
        st.info("Using ollama right now")
    else:
        ...
        #     or st.session_state.chat_history[-1]["content"] != full_response
        # ):
...
def render_personalized_feedback(player_profile):
    ...
    with col1:
        st.metric(
            "Points Per Game", f"{player_profile['stats']['points']['average']:.1f}"
        )
        st.metric(
            "Rebounds Per Game", f"{player_profile['stats']['rebounds']['average']:.1f}"
        )

    with col2:
        st.metric(
            "Assists Per Game", f"{player_profile['stats']['assists']['average']:.1f}"
        )
        st.metric(
            "Turnovers Per Game",
            f"{player_profile['stats']['turnovers']['average']:.1f}",
        )
